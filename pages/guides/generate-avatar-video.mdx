...
| Model | ID | Best for |
|-------|-----|----------|
| **Hedra Avatar** | `26f0fc66-152b-40ab-abed-76c43df99bc8` | Talking-head videos, lip-sync, up to 200s |
...
The audio determines the video length. For a 5-minute video, provide ~5 minutes of audio.
...
curl -X POST https://api.hedra.com/web-app/public/assets/{audio_asset_id}/upload \
  -H "X-API-Key: $HEDRA_API_KEY" \
  -F "file=@/path/to/five-minute-audio.mp3"
```
...
<Note>
  Set `duration_ms` to `300000` for 5 minutes (300 seconds = 300,000 ms). The video duration will match your audio length.
</Note>
