...
<Note>
  Hedra keeps generated media for at least **48 hours** after a job completes.
  Download `outputs[].url`, or pass `outputs[].asset_id` to the next job, soon
  after delivery. A delivery replayed after that window can carry `EXPIRED`
  outputs with `url: null`.
</Note>
