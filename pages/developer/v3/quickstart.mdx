...
models means changing the model id and choosing values the new model's schema
accepts. Browse the [model catalog](https://www.hedra.com/develop/models) to
pick one.
...
`GET /models` lists every model with its `id` and display `name`, such as
