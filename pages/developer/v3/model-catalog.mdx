...
3. **Estimate the cost.** `POST /v3/models/{id}/estimate` returns the price in
   US dollars for a specific request body before you run it.
4. **Submit the job.** `POST /v3/models/{id}` returns a `job_id`; poll it with
   ...
   The status response also carries the job's estimated completion time; see
   [Job progress](/pages/developer/v3/job-progress).
