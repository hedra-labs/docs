...
field-level error before you submit. Every image model also publishes
`"pattern": "\\S"` on `text_prompt`, so a whitespace-only prompt fails
validation instead of reaching the provider.
