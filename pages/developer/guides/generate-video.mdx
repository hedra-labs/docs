...
| `generated_video_inputs.seed` | Integer seed for reproducible output on models that support it (`fal/veo-2-t2v`, every Veo 3 and Veo 3 Fast slug, and all three `fal/qwen-happy-horse-*` slugs). Omit to get a fresh random seed on each call. Not accepted on `fal/veo-2-i2v`. |
