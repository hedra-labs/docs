...
description: "Generate videos from text prompts or start images with the Hedra API, covering text-to-video, image-to-video, duration, and Kling cfg_scale tuning."
---
...
| `generated_video_inputs.cfg_scale` | Prompt-adherence knob accepted by Kling video models |
| `batch_size` | Generate 1-8 variations at once (default: 1) |

<Note>
  `cfg_scale` is only accepted by video models whose provider documents it
  (Kling 1.6, 2.1, 2.5 Turbo, 2.6, and 3.0 endpoints). Each accepted model
  publishes its own allowed range on its input schema — call
  `GET /models?types=video` and read the range from the target slug's schema
  before setting a value. Sending a value outside that range returns a 400,
  and sending `cfg_scale` to a model that doesn't accept it is also rejected.
  Omitting it falls back to the provider's own default. This knob doesn't
  change the billed price on any model.
</Note>
