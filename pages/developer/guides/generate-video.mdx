...
| `generated_video_inputs.resolution` | Output resolution, e.g. `"480p"`, `"720p"`. Each model has its own default when omitted (Grok Video renders and bills at the published `"480p"` default). A value that isn't on a model's menu falls back to that same default rather than failing the submit. |
