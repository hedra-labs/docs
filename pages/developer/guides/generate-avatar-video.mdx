...
limits, input file-size caps, and image-dimension bounds before submitting
long-form work.
...
<Note>
  ...
  `max_dimension_px`). Read both before uploading. Hedra Avatar currently
  truncates audio longer than the model's output ceiling rather than rejecting
  it, so audio-duration bounds are intentionally not published — pre-trim your
  audio to the length you want billed.
</Note>
